...
export function ImportacoesTabContent() {
  ...
  const handlePublishAsset = (assetId: string) => {
    store.approveImportAsset(assetId);
    setAssets(store.getImportAssets());
    showNotification(`Asset #${assetId.substring(0, 8)} aprovado e publicado no acervo com sucesso!`);
  };
        ...
        {pendingAssets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pendingAssets.map((asset) => (
              <div
                key={asset.id}
                className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 flex gap-4 items-start"
              >
                <div className="w-24 h-24 rounded-xl bg-slate-200 dark:bg-slate-800 overflow-hidden shrink-0 border border-slate-300 dark:border-slate-700">
                  <img
